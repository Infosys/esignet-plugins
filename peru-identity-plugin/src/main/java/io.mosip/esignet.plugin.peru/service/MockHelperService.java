...
public class MockHelperService {
    ...
    public KycExchangeResult kycExchange(String relyingPartyId, String clientId, KycExchangeRequestDto kycExchangeRequestDto) throws KycExchangeException {
        ...
        if(result==null || !result.getValidity().equals(Valid.ACTIVE)){
        ...
        try {
            Map<String, Object> kyc =buildKycDataBasedOnPolicy(kycExchangeRequestDto.getKycToken(),
                    kycExchangeRequestDto.getAcceptedClaims());
    ...
    private KycAuthResult validateKnowledgeBasedAuth(String individualId, AuthChallenge authChallenge) throws KycAuthException {
        ...
        try {
            ...
            && envelope.getBody().getConsultarResponse().getResponseReturn().getDatosPersona()!=null){
                ...
                boolean authStatus=validateKnowledgeBasedAuth(authChallenge.getChallenge(),datosPersona);
                if(authStatus){
                    String kycToken = generateB64EncodedHash(ALGO_SHA3_256, UUID.randomUUID().toString());
                    kycAuthResult.setKycToken(kycToken);
    ...
    private boolean validateKnowledgeBasedAuth(String encodedChallenge,DatosPersona datosPersona) throws KycAuthException {
    ...
    private Map<String, Object> buildKycDataBasedOnPolicy(String kycToken, List<String> claims) throws KycExchangeException {
        ...
        for (String claim : claims) {
            switch (claim) {
                case "name":
                    if (persondata.getApellidoCasada() != null) {
                        kyc.put("name", persondata.getPrenombres());
                    }
                    break;
                case "gender":
                    if(persondata.getEstadoCivil()!=null){
                       kyc.put("gender",persondata.getGenero());
                    }
                    break;
                case "given_name":
                    if(persondata.getEstadoCivil()!=null){
                        kyc.put("given_name",persondata.getPrimerApellido());
                    }
                    break;
                case "birthdate":
                    if(persondata.getEstadoCivil()!=null){
                        kyc.put("birthdate",persondata.getFechaNacimiento());
                    }
                    break;
            }
        }
